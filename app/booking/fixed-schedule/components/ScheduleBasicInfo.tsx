...
import type { FixedSchedulePreviewRequest, Court } from '../types';
...
/**
 * Tính endDate từ startDate + số tuần.
 * endDate = startDate + (weeks - 1) * 7
 * → Buổi cuối rơi đúng vào tuần thứ `weeks`
 */
function calcEndDate(startDateStr: string, weeks: number): string {
  const start = new Date(startDateStr);
  // (weeks - 1) * 7 ngày để buổi đầu tính là tuần 1
...
export function ScheduleBasicInfo({ onPreview, loading }: Props) {
  ...
  const [weeklySlots, setWeeklySlots] = useState<WeeklySlotLocal[]>([
  ...
  const toggleDay = (dayOfWeek: number) => {
    setWeeklySlots((prev) => {
      ...
      if (exists) {
        if (prev.length === 1) return prev; // phải có ít nhất 1 slot
        return prev.filter((s) => s.dayOfWeek !== dayOfWeek);
      }
      return [
        ...prev,
        { dayOfWeek, timeStart: '08:00', timeEnd: '10:00' },
      ].sort((a, b) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // FIX: convert numberOfWeeks → endDate và lấy giờ từ slot đầu tiên
    // BE chỉ nhận 1 cặp (timeStart, timeEnd) + (startDate, endDate)
    const firstSlot = weeklySlots[0];
    const endDate   = calcEndDate(startDate, numberOfWeeks);

    const payload: FixedSchedulePreviewRequest = {
      courtId:   parseInt(courtId),
      cycle:     'weekly',          // ScheduleBasicInfo chỉ hỗ trợ weekly
      startDate,
      endDate,                      // ← FIX: BE cần endDate, không nhận numberOfWeeks
      timeStart: firstSlot.timeStart,
      timeEnd:   firstSlot.timeEnd,
    };

    console.log('📤 ScheduleBasicInfo → onPreview payload:', payload);
    onPreview(payload);
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 space-y-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Preview endDate để user biết */}
        {startDate && (
          <p className="text-xs text-gray-400 -mt-2">
            📅 Ngày kết thúc ước tính:{' '}
            <span className="font-medium text-gray-600">
              {calcEndDate(startDate, numberOfWeeks)}
            </span>
          </p>
        )}
          ...
          <div className="space-y-2">
            {weeklySlots.map((slot) => {
              ...
              return (
                ...
                >
                  <span className="text-xs font-bold text-blue-700 w-6 text-center">
                    {dayInfo?.short}
                  </span>
                  <input
                    type="time"
                    ...
                    onChange={(e) =>
                      updateSlotTime(slot.dayOfWeek, 'timeStart', e.target.value)
                    }
                    className={cn(
                      'flex-1 h-8 px-2 border rounded text-sm',
                  ...
                  <input
                    ...
                    value={slot.timeEnd}
                    onChange={(e) =>
                      updateSlotTime(slot.dayOfWeek, 'timeEnd', e.target.value)
                    }
                    className={cn(
                      'flex-1 h-8 px-2 border rounded text-sm',
                      hasError ? 'border-red-300' : 'border-gray-200',
