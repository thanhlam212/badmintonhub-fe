...
export default function VNPayReturnPage() {
  ...
  useEffect(() => {
    ...
    if (amountRaw) setAmount(parseInt(amountRaw) / 100)
    ...
    if (responseCode === "00") {
      ...
      setMessage(errorMessages[responseCode] || `Thanh toán thất bại (mã lỗi: ${responseCode})`)
    } else {
      setStatus("failed")
            ...
            <div className="rounded-lg border p-4 space-y-2 bg-muted/30">
              {orderInfo && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-right max-w-[60%]">{orderInfo}</span>
                </div>
              )}
              {amount && (
                <div className="flex justify-between text-sm">
